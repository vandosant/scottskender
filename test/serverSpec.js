...
It was a Stratocaster with a whammy bar`})
  ...
  request(app)
    ...
    it('should update a post', function(done) {
      createDocument(User, {username: 'norman', password: 'graycoat-soldier'})
        .then(function(user) {
          return createDocument(Post, {title: 'i should blame you now, but i never could somehow',
          content: 'for a miner\'s wife you weren\'t cut out to be', author: user._id})
    	  .then(function(post) {
	                                                                request(app)
	    .post('/auth/signin')
	    .send({username: 'norman', password: 'graycoat-soldier'})
	    .set('Accept', 'application/json')
	    .expect('Content-Type', /json/)
	    .expect(200)
	    .end(function (err, res) {
  return request(app)
    ...
    it('requires same author to update a post', function(done) {
      createDocument(User, {username: 'dio', password: 'holy-diver'})
       .then(function(userDio) {
         return createDocument(User, {username: 'iommi', password: 'gibson-sg'})
           .then(function (userIommi) {
	                                                                                                                                       return createDocument(Post, {title: 'children of the sea', content: `In the misty morning, on the edge of time
We've lost the rising sun, a final sign
As the misty morning rolls away to die
Reaching for the stars, we blind the sky`, author: userDio._id})
		.then(function (post) {
  request(app)
		    .post('/auth/signin')
		    .send({username: 'iommi', password: 'gibson-sg'})
		    .expect(200)
		    .end(function(err, res) {
  request(app)
			.put(`/api/posts/${post._id}`)
			.send({content: 'Dun dun dun dun dun dun'})
			.set('Accept', 'application/json')
			.set('Authentication', 'Bearer {res.body.token}')
			.expect(401, done);
		    });
		});
	                                                                                                                 });
